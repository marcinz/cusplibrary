              ...
              cusp::coo_format)
{
    ...
    // workspace arrays
    typedef cusp::detail::temporary_array<IndexType, DerivedPolicy> temp_index_array_t;
    typedef typename temp_index_array_t::iterator temp_index_array_iter_t;
    typedef cusp::detail::temporary_array<ValueType, DerivedPolicy> temp_value_array_t;
    typedef typename temp_value_array_t::iterator temp_value_array_iter_t;
    temp_index_array_t A_gather_locations(exec);
    temp_index_array_t B_gather_locations(exec);
    temp_index_array_t I(exec);
    temp_index_array_t J(exec);
    temp_value_array_t V(exec);
    ...
    if (coo_num_nonzeros <= workspace_capacity)
    {
        ...
        coo_spmm_helper(exec,
                        ...
                        I, J, V,
                        initialize, combine, reduce, accum);
    }
    else
    {
        ...
        temp_index_array_t A_row_offsets(exec, A.num_rows + 1);
        cusp::indices_to_offsets(exec, A.row_indices, A_row_offsets);

        // compute worspace requirements for each row
        temp_index_array_t cummulative_row_workspace(exec, A.num_rows);
        thrust::gather(exec,
        ...
        const size_t C_previous_size = C.values.size();

        // resize output to store the old and the new entries
        C.resize(A.num_rows, B.num_cols, C_num_entries + C_previous_size);

        // copy slices into output after the original entries
        size_t base = C_previous_size;
        for(typename ContainerList::iterator iter = slices.begin(); iter != slices.end(); ++iter)
            {
        ...
        // if there were entries in the matrix before the multiplication, we have to merge them with the new entries
        if (C_previous_size != 0) {
            // First sort the entries
            C.sort_by_row_and_column();
            // compute unique number of nonzeros in the output
            const IndexType NNZ = thrust::inner_product(
                exec,
                thrust::make_zip_iterator(thrust::make_tuple(C.row_indices_begin(), C.row_indices_begin())),
                thrust::make_zip_iterator(thrust::make_tuple(C.row_indices_end (),  C.row_indices_end()))    - 1,
                thrust::make_zip_iterator(thrust::make_tuple(C.row_indices_begin(), C.row_indices_begin()))  + 1,
                IndexType(0),
                thrust::plus<IndexType>(),
                thrust::not_equal_to< thrust::tuple<IndexType,IndexType> >()) + 1;
            // prepare the temporaries to handle up to the total size of the matrix elements
            I.resize(NNZ);
            J.resize(NNZ);
            V.resize(NNZ);
            // Now reduce duplicate entries into tomporary storage
            thrust::reduce_by_key(
               exec,
               thrust::make_zip_iterator(thrust::make_tuple(C.row_indices_begin(), C.column_indices_begin())),
               thrust::make_zip_iterator(thrust::make_tuple(C.row_indices_end(), C.column_indices_end())),
               C.values_begin(),
               thrust::make_zip_iterator(thrust::make_tuple(I.begin(), J.begin())),
               V.begin(),
               thrust::equal_to< thrust::tuple<IndexType,IndexType> >(),
               accum);
            // Finally, swap temporary storage with the matrix storage
            thrust::swap(C.row_indices, I);
            thrust::swap(C.column_indices, J);
            thrust::swap(C.values, V);
